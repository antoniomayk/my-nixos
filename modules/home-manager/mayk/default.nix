    ...
    { pkgs, ... }:
    {
      ...
      services.mako = {
        ...
        settings = {
          ...
          border-color = "${colors.ifoc}";
          text-color = "${colors.fg}";
          progress-color = "${colors.inn}";
        ...
        in
        {
          ...
          config = {
            ...
            colors = {
              ...
              focused = {
                border = colors.afoc;
                background = colors.afoc;
                text = colors.bg;
                indicator = colors.inn;
                childBorder = colors.afoc;
              };
              focusedInactive = {
                border = colors.ifoc;
                background = colors.ifoc;
                text = colors.bg;
                indicator = colors.inn;
                childBorder = colors.ifoc;
              };
              unfocused = {
                border = colors.ufoc;
                background = colors.ufoc;
                text = colors.fg;
                indicator = colors.inn;
                childBorder = colors.ufoc;
              };
              urgent = {
                border = colors.urgt;
                background = colors.urgt;
                text = colors.fg;
                indicator = colors.inn;
                childBorder = colors.urgt;
              };
              placeholder = {
                border = colors.phol;
                background = colors.phol;
                text = colors.fg;
                indicator = colors.inn;
                childBorder = colors.phol;
              };
            };
            keybindings = {
      ...
      programs.i3status = {
        ...
        general = {
          ...
          colors = true;
          interval = 5;
          color_good = "${colors.afoc}";
          color_bad = "${colors.urgt}";
          color_degraded = "${colors.warn}";
          markup = "pango";
        };
        modules = {
          "volume master" = {
            position = 0;
            settings = {
              format = "<span bgcolor='${colors.bg}' foreground='${colors.fg}'> 󰕾  %volume</span>";
              format_muted = "<span bgcolor='${colors.bg}' foreground='${colors.urgt}'> 󰝟  %volume</span>";
              device = "default";
              mixer = "Master";
              mixer_idx = 0;
            };
          ...
          "wireless _first_" = {
            position = 1;
            settings = {
              format_up = "<span bgcolor='${colors.bg}' foreground='${colors.inn}'> 󰖩 %quality %essid [%ip]</span>";
              format_down = "<span bgcolor='${colors.bg}' foreground='${colors.urgt}'> 󰖪  Disconnected</span>";
            };
          };
          "ethernet _first_" = {
            position = 2;
            settings = {
              format_up = "<span bgcolor='${colors.bg}' foreground='${colors.inn}'> 󰈁 %ip</span>";
              format_down = "<span bgcolor='${colors.bg}' foreground='${colors.urgt}'> 󰈂 Disconnected</span>";
            };
          };
          "disk '/'" = {
            position = 3;
            settings = {
              format = "<span bgcolor='${colors.bg}' foreground='${colors.fg}'> 󰋊  %avail</span>";
              prefix_type = "custom";
            };
          };
          # memory = {
          #   position = 4;
          #   settings = {
          #     format = "<span bgcolor='${colors.bg}' foreground='${colors.fg}'> 󰍛  %used</span>";
          ...
          #   position = 5;
          #   settings = {
          #     format = "<span bgcolor='${colors.bg}' foreground='${colors.fg}'> 󰘚  %usage</span>";
          #   };
          # };
          "tztime localtime" = {
            position = 6;
            settings = {
              format = "<span bgcolor='${colors.bg}' foreground='${colors.fg}'> 󰸗  %a %d-%m-%Y %I:%M %p</span>";
            };
          };
        };
      };
        ...
        theme =
          ...
          in
          {
            "*" = {
              ...
              active-foreground = mkLiteral "${colors.afoc}";
              alternate-active-background = mkLiteral "${colors.bg}";
              alternate-active-foreground = mkLiteral "@active-foreground";
              alternate-normal-background = mkLiteral "${colors.bg}";
              alternate-normal-foreground = mkLiteral "@foreground";
              alternate-urgent-background = mkLiteral "${colors.bg}";
              ...
              border-color = mkLiteral "${colors.ifoc}";
              foreground = mkLiteral "${colors.fg}";
              normal-background = mkLiteral "${colors.bg}";
              normal-foreground = mkLiteral "@foreground";
              selected-active-background = mkLiteral "${colors.afoc}";
              selected-active-foreground = mkLiteral "${colors.bg}";
              selected-normal-background = mkLiteral "${colors.afoc}";
              selected-normal-foreground = mkLiteral "${colors.bg}";
              selected-urgent-background = mkLiteral "${colors.urgt}";
              selected-urgent-foreground = mkLiteral "${colors.bg}";
              separatorcolor = mkLiteral "${colors.ifoc}";
              spacing = mkLiteral "0";
              urgent-background = mkLiteral "${colors.bg}";
              urgent-foreground = mkLiteral "${colors.urgt}";
            };
            "window" = {
